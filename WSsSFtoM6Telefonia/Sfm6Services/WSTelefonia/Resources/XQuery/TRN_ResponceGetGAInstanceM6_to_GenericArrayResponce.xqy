...
declare function local:func($Input as element() (:: schema-element(ns1:getGAInstanceResponse) ::)) as element() (:: schema-element(ns2:Root-Element) ::) {
    <ns2:Root-Element>
        {
            for $gaInstance in $Input/res:buscarGAInstanceResponse/res:buscarGAInstanceByIdResponseArray/mod:gaInstance
            return 
            <ns2:topLevelArray>
                <ns2:label>{fn:data($gaInstance/gai:instanceValue)}</ns2:label>
                <ns2:value>{fn:data($gaInstance/gai:gaInstanceID)}</ns2:value></ns2:topLevelArray>
        }
